const SHEET_ID = "1eWkRl_E_PCYWZ_EGRo7_ZC7gP5XlAC_bFVeH2VhD4Tc";
...
// 1. ฟังก์ชันเปิดหน้าเว็บ (เมื่อเข้าลิงก์ Web App)
function doGet() {
...
// 2. ฟังก์ชันส่งข้อมูลทั้งหมดในชีทกลับไปที่หน้าเว็บ (รวมทั้งกิจกรรมและตัวชี้วัด KPI)
...
// 3. ฟังก์ชันรับข้อมูลจากฟอร์มเพื่อบันทึกลงชีท
function saveActivityData(data) {
  try {
    ...
    // --- 🛠️ แก้ไข: หาแถวสุดท้ายที่แท้จริงที่มีข้อมูล (อ้างอิงจากคอลัมน์ A) ---
    var columnA = sheet.getRange("A:A").getValues();
    var trueLastRow = 0;
    
    // วนลูปเช็กจากล่างขึ้นบน หาช่องแรกที่ไม่ว่าง
